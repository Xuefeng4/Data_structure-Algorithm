...
bool isOrderedRecursiveHelper(Node* node, T low, T high) const;
void binaryTreePaths(std::vector<std::vector<T>>& paths, Node* root, std::vector<T>cur)const ;

int  binaryTreePathsSum(Node* root, int len)const ;
