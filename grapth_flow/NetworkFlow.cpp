...
NetworkFlow::NetworkFlow(Graph & startingGraph, Vertex source, Vertex sink) :
  ...
  // YOUR CODE HERE
  maxFlow_ = 0;
  std::vector<Vertex> vertexlist = startingGraph.getVertices();
  for(auto it:vertexlist){
    flow_.insertVertex(it);
    residual_.insertVertex(it);
  }
  std::vector<Edge> edgelist = startingGraph.getEdges();
  for(auto it:edgelist)
  {
    ...
    residual_.setEdgeWeight(it.source, it.dest,startingGraph.getEdgeWeight(it.source, it.dest));
...
int NetworkFlow::pathCapacity(const std::vector<Vertex> & path) const {
  ...
  for(unsigned i = 0; i<path.size()-1;i++){
    int tmp = residual_.getEdgeWeight(path[i],path[i+1]);
    if(tmp <= cap) cap = tmp;
  }
  return cap;
}
...
const Graph & NetworkFlow::calculateFlow() {
  ...
  while(findAugmentingPath(source_, sink_, path)){
    ...
    for(unsigned i =0; i<path.size()-1; i++){
      ...
      residual_.setEdgeWeight(path[i+1],path[i],residual_.getEdgeWeight(path[i],path[i+1])+cap);
      }
    }
  return flow_;
}
