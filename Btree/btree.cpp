...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
     ...
     // if(subroot->elements[idx] == key) return subroot->elements[idx].value;
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    //insertion_idx(parent.children, key);
    //BTreeNode * newParent = new_right;
    parent->children.insert(child_itr,new_right);//child->elements[mid_elem_idx
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    size_t first_larger_idx = insertion_idx(subroot->elements, pair);
      /* TODO Your code goes here! */
    if(subroot->is_leaf) {
      //if(std::find(subroot->elements.begin(),subroot->elements.end(),pair.key)!= subroot->elements.end()){
        if(subroot->elements[first_larger_idx] == pair.key && !subroot->elements.empty() &&
