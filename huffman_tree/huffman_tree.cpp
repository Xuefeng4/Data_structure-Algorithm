                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
     ...
     if(singleQueue.empty()&&mergeQueue.empty()) return NULL;
       ...
       small1 = singleQueue.front();
       small2 = mergeQueue.front();
       if (small1->freq< small2->freq){
         singleQueue.pop();
         return small1;
       }
       else {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     for(unsigned i = 0; i <frequencies.size();i++){
       Frequency tmp = frequencies[i];
       TreeNode* leaf = new TreeNode(tmp);
       singleQueue.push(leaf);
     }

     while(singleQueue.size() + mergeQueue.size()!= 1){//||(singleQueue.size()==0 && mergeQueue.size()==1))){
       TreeNode* small1 = removeSmallest(singleQueue,mergeQueue);
       TreeNode* small2 = removeSmallest(singleQueue,mergeQueue);
       int freq_total = (small1->freq).getFrequency() + (small2->freq).getFrequency();
       TreeNode *internal = new TreeNode(freq_total);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.hasBits()){
      ...
      if(current == 1){
        ...
        TreeNode * newinternal = new TreeNode(Frequency(0));
