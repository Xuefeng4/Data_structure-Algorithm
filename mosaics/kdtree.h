...
class KDTree
{
  ...
  private:
     ...
     */
     std::vector<Point<Dim>> nodeList;
     KDTreeNode * sortDim(int curDim, int left, int right);
     int quickSelect(int curDim,int left, int right, int key);
     int _partition(int curDim, int left, int right);
     KDTreeNode* _copy(const KDTreeNode * otherNode);
     void _destory(const KDTreeNode * node);
     void findTarget(const Point<Dim>& query, KDTreeNode * node, int curDim, Point<Dim> *& curBest) const;
     Point<Dim> findNearestNeighbor(const KDTreeNode *node, const Point<Dim>& query, const int dim) const;
